...
userinit(void)
{
  ...
  p->tickets = 1; //Set one ticket for the first user process
...
getpinfo(uint64 addr){
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    if(p->state == UNUSED) pstat1.inuse[i] = 0;
    else{
        pstat1.inuse[i] = 1;
        pstat1.tickets[i] = p->tickets;
        pstat1.pid[i] = p->pid;
        pstat1.ticks[i] = p->ticks;
    }
    i++;
  }
  return copyout(myproc()->pagetable, addr, (char *)&pstat1, sizeof(pstat1));
}
